...
<html lang="en">
	<head>
		...
		<link rel="stylesheet" type="text/css" href="./style/hot_style.css">
	...
	<body>
		...
		<div id="topSongsList">
			<h3 class='moveToRight' id="h3">Popular Music Videos from Youtube:</h2>
			<?php
					...
					$songIDtitle = array();
					for ($i = 0; $i < 50; $i++){ 
						$songIDtitle[$i] = mysql_fetch_array($query);

						$videoId[$i] = $songIDtitle[$i][0];
						$thumbnail[$i] = "http://img.youtube.com/vi/$videoId[$i]/default.jpg";
						$videoTitle[$i] = $songIDtitle[$i][1];

						$videoTitle[$i] = str_replace("(", "", $videoTitle[$i]);
						$videoTitle[$i] = str_replace(")", "", $videoTitle[$i]);
						$videoTitle[$i] = str_replace("[", "", $videoTitle[$i]);
						$videoTitle[$i] = str_replace("]", "", $videoTitle[$i]);
						$videoTitle[$i] = str_replace("Audio", "", $videoTitle[$i]);
						$videoTitle[$i] = str_replace("Official", "", $videoTitle[$i]);
						$videoTitle[$i] = str_replace("Official Video", "", $videoTitle[$i]);
						$videoTitle[$i] = str_replace("Music Video", "", $videoTitle[$i]);
						$videoTitle[$i] = str_replace("Music", "", $videoTitle[$i]);
						$videoTitle[$i] = str_replace("Video", "", $videoTitle[$i]);
						$videoTitle[$i] = str_replace("Explicit", "", $videoTitle[$i]);
						$videoTitle[$i] = str_replace(" And ", " & ", $videoTitle[$i]);

						$i2 = $i + 1;

						echo "<a href=\"watch?v=$videoId[$i]\"><div id=\"song[$i]\" class=\"songList\">
					$i2.&nbsp;&nbsp;<img src=\"$thumbnail[$i]\">
					<span><b>$videoTitle[$i]</b></span>
				</div></a>
				";
					}
